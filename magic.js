...
(function(){
  ...
  function star(x,y,r,rot,a,base){
    // crisp 5-point star so it reads clearly as a star, not a speck
    ctx.save(); ctx.translate(x,y); ctx.rotate(rot);
    ctx.fillStyle=col(base,a); ctx.shadowBlur=9; ctx.shadowColor=col(base,a*0.9);
    ctx.beginPath();
    for(var i=0;i<5;i++){
      var o=(i*2*Math.PI/5)-Math.PI/2;               // outer point
      ctx.lineTo(Math.cos(o)*r, Math.sin(o)*r);
      var inr=o+Math.PI/5;                            // inner notch
      ctx.lineTo(Math.cos(inr)*r*0.42, Math.sin(inr)*r*0.42);
    }
  ...
  function burst(x,y,n,spread){
    ...
    for(var i=0;i<n;i++){
      ...
      parts.push({x:x,y:y, vx:Math.cos(ang)*sp, vy:Math.sin(ang)*sp - 0.6, g:0.045+Math.random()*0.03,
        r:Math.random()*6+5, rot:Math.random()*6.28, vr:(Math.random()-0.5)*0.3,
        life:0, max:38+Math.random()*34, a:0.9, base:base, kind:'star'});
    }
    run();
  }
  ...
  window.addEventListener('scroll',function(){
    ...
    if(acc>140){ acc=0;
      ...
      for(var i=0;i<n;i++){
        ...
        // twinkle nearly in place (barely drifts) so it reads as a star, not a crawling bug
        parts.push({x:x,y:yy, vx:(Math.random()-0.5)*0.08, vy:-(Math.random()*0.08+0.02), g:0,
          r:Math.random()*5+4, rot:Math.random()*6.28, vr:(Math.random()-0.5)*0.05,
          life:0, max:66+Math.random()*40, a:0.5, base:Math.random()<0.7?GOLD:CREAM, kind:'star'});
      }
      run();
    }
  }, {passive:true});
